...
class Pagex_Coming_Soon {
	...
	public function __construct() {
		...
		if ( $active == 'yes' && ! $page ) {
			...
			add_filter( 'template_include', array( $this, 'page_template' ), 99999 );
		}
	}
	...
	public function page_template( $template ) {
		...
		$template_page_id = Pagex::get_translation_id( $this->id, 'pagex_layout_builder' );
		add_action( 'pagex_post_content', function () use ( $template_page_id ) {
			while ( have_posts() ) {
				the_post();
				echo apply_filters( 'pagex_content', do_shortcode( get_post_field( 'post_content', $template_page_id ) ) );
			}
		} );
