...
function pagex_register_heading_element( $elements ) {
	$template = '<% if (data.link != "") { %><a class="pagex-static-link" <%= data.link %>> <% } %><<%= data.tag %> class="pagex-heading pagex-lang-str" contenteditable="true"><% if (data.content && data.content.length) { print(data.content) } else { print("' . __( 'Heading or small paragraph', 'pagex' ) . '")  } %></<%= data.tag %>><% if (data.link != "") { %></a><% } %>';
