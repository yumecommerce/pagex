...
class Pagex_Editor {
	...
	public function controls( $controls ) {
		...
		$controls['column-options'] = '<div class="pagex-options pagex-column-options btn-group btn-group-sm"><button type="button" class="btn pagex-tooltip" title="' . __( 'Edit Column', 'pagex' ) . '" data-edit="column"><i class="fas fa-pen"></i></button><button type="button" class="btn pagex-tooltip append" title="' . __( 'Append New Element', 'pagex' ) . '" data-add="element"><i class="fas fa-plus-circle"></i></button><button type="button" class="btn pagex-tooltip prepend" title="' . __( 'Prepend New Element', 'pagex' ) . '" data-add="element"><i class="fas fa-plus-square"></i></button><button type="button" class="btn pagex-tooltip" title="' . __( 'Clone Column', 'pagex' ) . '" data-clone="column"><i class="fas fa-clone"></i></button><button type="button" class="btn pagex-tooltip" title="' . __( 'Remove Column', 'pagex' ) . '" data-remove="column"><i class="fas fa-trash"></i></button><button type="button" class="btn pagex-options-toggle pagex-tooltip" title="' . __( 'Hide/Show Options', 'pagex' ) . '"></button></div>';

		$controls['add-element-options'] = '<div class="pagex-options pagex-add-element-options"><button type="button" class="btn pagex-tooltip" title="' . __( 'Add Element', 'pagex' ) . '" data-add="element"><i class="fas fa-plus-circle"></i></button></div>';
		...
		$controls['option-start'] = '<% var isResp = !_.isUndefined(data.responsive) || data.type == "dimension"; %><div class="pagex-control-wrapper <%- data.class ? data.class : "col-12"%> <%- data.hidden ? "pagex-hide" : ""%> <% if (data.action) { print("pagex-control-action-type-" + data.action) } %>" <% if (!_.isUndefined(data.condition)) { %>data-condition="<%- JSON.stringify(data.condition) %>"<% } %>><div class="form-group"><% if (data.title) { %><label class="<%- isResp ? "pagex-responsive-label" : "" %>"><%- data.title %></label><% } %><% if (isResp) { %><div class="pagex-responsive-switchers"><div class="d-inline ml-1"><span class="pagex-responsive-switcher-button badge badge-secondary trn-300"><i class="far fa-window-restore"></i></span></div></div> <% } %>';
		...
		$controls['link'] = '<input class="pagex-control-field pagex-link-control pagex-hide" type="text" name="<%- data.id %>"><div class="mb-2"><div class="input-group"><input data-condition=\'{"<%- data.id %>-dynamic":[""]}\' class="pagex-control-field form-control pagex-link-control-field pagex-link-control-href" type="text" placeholder="Url" name="<%- data.id %>-href"><select class="custom-select pagex-control-option pagex-link-control-field pagex-link-control-dynamic" name="<%- data.id %>-dynamic">' . pagex_get_dynamic_url_keys() . '</select><div class="input-group-append"><button type="button" data-condition=\'{"<%- data.id %>-dynamic":[""]}\' class="btn pagex-link-control-search">' . __( 'Search', 'pagex' ) . '</button><button type="button" class="btn pagex-link-show-attrs"><i class="fas fa-cog"></i></button></div></div><div class="pagex-link-search-results"></div></div><div class="form-row align-items-center pagex-link-attrs pagex-hide"><div class="col-8 mb-2"><div class="input-group"><input class="pagex-control-field form-control pagex-link-control-field pagex-link-control-onclick" type="text" placeholder="onclick" name="<%- data.id %>-onclick"><div class="input-group-append"><a href="https://github.com/yumecommerce/pagex/wiki/onclick-Actions" target="_blank" class="btn"><i class="fas fa-info"></i></a></div></div></div><div class="col-4 mb-2"><input class="pagex-control-field form-control pagex-link-control-field pagex-link-control-title" type="text" placeholder="' . __( 'Title', 'pagex' ) . '" name="<%- data.id %>-title"></div><div class="col-auto"><div><label class="m-0"><input class="pagex-control-option pagex-link-control-field pagex-link-control-rel" type="checkbox" value="pagex-checkbox-true" name="<%- data.id %>-rel"> <span>Nofollow</span></label></div></div><div class="col-auto"><div><label class="m-0"><input class="pagex-control-option pagex-link-control-field pagex-link-control-target" type="checkbox" value="pagex-checkbox-true" name="<%- data.id %>-target"> ' . __( 'Open in a new tab', 'pagex' ) . '</label></div></div></div>';
...
//			'pricing'        => __( 'Pricing', 'pagex' ),
		...
		if ( is_admin() || Pagex::is_frontend_builder_active() ) {
			...
			// main modal form
			echo '<div id="pagex-params-modal" class="pagex-params-modal pagex-hide"><div class="pagex-params-modal-head pagex-params-modal-params-move"><div id="pagex-params-modal-title"></div><div class="pagex-params-modal-controls"><div class="pagex-params-modal-help"><a href="" id="pagex-element-info-link" target="_blank" title="' . __( 'Info About Element', 'pagex' ) . '"><i class="fas fa-info"></i></a></div><div class="pagex-params-modal-close"><i class="fas fa-times"></i></div></div></div><form id="pagex-params-form"></form></div>';
		}
	}
